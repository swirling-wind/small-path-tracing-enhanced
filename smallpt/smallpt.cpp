...
{
	//1644328928
	std::time_t result = std::time(nullptr); // generate the num of seconds after 1970-1-1
	std::cout << result << std::endl;

	std::string ret;
	ret.resize(64);
	int wsize = sprintf((char*)ret.data(), "%s", std::ctime(&result));
	ret.resize(wsize);

	auto file_name = replace_all(ret, ":", "-");
	file_name = replace_all(ret, "\n", "");
	file_name = replace_all(ret, "  ", ",");
	file_name = replace_all(ret, " ", ",");
	return ret;
}
...
//fprintf(stderr,"\rRendering (%d spp) %5.2f%%",samps*4,100.*y/(h-1));
	...
	std::string title = getCurrentTime() + ".ppm";
	std::cout << title;
	FILE* f = fopen(title.c_str(), "w");         // Write image to PPM file.
	fprintf(f, "P3\n%d %d\n%d\n", w, h, 255);
	for (int i = 0; i < w * h; i++)
